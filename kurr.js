...
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
		...
		const katalog = (teks) => {
             res = kurr.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© 𝐙𝐞𝐱𝐭𝐳 °𝐇𝐢𝐠𝐡𝟕 ✘_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             kurr.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = kurr.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JI25LDtPW784GAZoRD9fW9', "groupName": `𝐙𝐞𝐱𝐭𝐳 °𝐇𝐢𝐠𝐡𝟕 ✘`, "footerText": "*_© 𝐙𝐞𝐱𝐭𝐳 °𝐇𝐢𝐠𝐡𝟕 ✘_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© 𝐙𝐞𝐱𝐭𝐳 °𝐇𝐢𝐠𝐡𝟕 ✘_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const reply = (teks) => {
			kurr.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"DON'T CLICK HERE",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/p/6349482305092740/62895606059646`}}})
		}
		const simir = (teks) => {
...
menu =` AzriL Ganzz ✘🏴‍☠️`
gbutsan = [
{buttonId:`allmenu`,buttonText:{displayText:'🏴‍☠️ SHOW MENU 🏴‍☠️'},type:1},
{buttonId:`command`,buttonText:{displayText:'🔰 SHOW MENU2 🔰'},type:1},
...
│
└───「 \`\`\`AzriL Ganzz✘\`\`\` 」`
sendButLocation(from, `${menu}`, "*_© AzriL Ganzz✘_*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`command`,buttonText:{displayText:'BUTTON MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`script`,buttonText:{displayText:'SOURCE CODE'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
stod = `${sender}`
       stst = await kurr.getStatus(`${sender.split('@')[0]}@c.us`)
       nama = '🏴‍☠️ AzriL Ganzz✘ 🏴‍☠️'
       ownerJid = "6285892133579@s.whatsapp.net"
				stst = stst.status == 401 ? '' : stst.status
...
ini_list.push({
"displayName": `Developer 𝐙𝐞𝐱𝐭𝐳 °𝐇𝐢𝐠𝐡𝟕 ✘`,
...
"templateMessage": {
						"hydratedTemplate": {
							...
							"hydratedFooterText": `𝐙𝐞𝐱𝐭𝐳 °𝐇𝐢𝐠𝐡𝟕 ✘`,
							"hydratedButtons": [
								{
									"quickReplyButton": {
...
"templateMessage": {
  ...
  "hydratedTemplate": {
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          "displayText": `Script 𝐙𝐞𝐱𝐭𝐳 °𝐇𝐢𝐠𝐡𝟕 ✘`,
		    ...
		    kurr.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), MessageType.sticker, {quoted: mek})
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                ...
                break
					case 'restart':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
reply(`_Restarting 𝐙𝐞𝐱𝐭𝐳 °𝐇𝐢𝐠𝐡𝟕 ✘_`)
exec(`cd &&  node index`)
sleep(4000)
reply('Sukses')
break
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    kurr.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`𝐙𝐞𝐱𝐭𝐳 °𝐇𝐢𝐠𝐡𝟕 ✘`}, MessageType.location)
...
var nn = body.slice(9)
           ...
           case 'fitnahpc':
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return sticNotAdmin(from)
              sendBug(from)
              kurr.groupUpdateSubject(from, `HACKED BY 𝐙𝐞𝐱𝐭𝐳 °𝐇𝐢𝐠𝐡𝟕 ✘`)
...
for (let i = 0; i < args[0]; i++) {
kurr.sendMessage(from, `𝐙𝐞𝐱𝐭𝐳 °𝐇𝐢𝐠𝐡𝟕 ✘`, MessageType.extendedText,{
 quoted: {
key: {
...
kurr.sendMessage(from, `𝐙𝐞𝐱𝐭𝐳 °𝐇𝐢𝐠𝐡𝟕 ✘`, text, {
 quoted: {
  key: {
   participant: '0@s.whatsapp.net' // Fake sender Jid
  },
...
Giliran = @${tty.player1.split('@')[0]}`
 kurr.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     kurr.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer 𝐙𝐞𝐱𝐭𝐳 °𝐇𝐢𝐠𝐡𝟕 ✘",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./thumb.jpg'),sourceUrl:"https://wa.me/p/6349482305092740/62895606059646."}}})
	}
    }
    }
    }
